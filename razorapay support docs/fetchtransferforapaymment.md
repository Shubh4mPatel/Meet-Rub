...
Use this endpoint to retrieve the collection of all transfers created on a specific Payment id. Once the settlement against the transfer is processed, a webhook notification `settlement.processed` is sent which contains a `recipient_settlement_id`. Know more about [sample webhook payloads](https://razorpay.com/docs/build/llm-docs/webhooks/route.md#settlement-processed).
...
```bash: CLI
razorpay route transfers fetch-by-payment pay_JMNiqyuoFoJcXb
```
