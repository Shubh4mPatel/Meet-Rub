...
Use this endpoint to create Transfers from captured payments. You can create and capture payments in the regular payments flow using the [Razorpay Checkout](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/standard.md) and [Payment APIs](https://razorpay.com/docs/build/llm-docs/api/payments.md#capture-a-payment).
...
```bash: CLI
razorpay route transfers create-from-payment pay_JMNiqyuoFoJcXb \
  --account acc_IEIkSOM5VJb2Lm \
  --amount 5000 \
  --currency INR \
  --linked-account-note order_id \
  --note purpose=split
```
...
: `array` Details regarding the transfer.
    ...
    `on_hold`
    : `boolean` Indicates whether the account settlement for transfer is on hold. Know more about [on hold settlements](https://razorpay.com/docs/build/llm-docs/payments/route/schedule-settlement.md#settlement-settings-for-linked-accounts). Possible values:
...
* solution: Make sure you have enough balance. You can also [add funds](https://razorpay.com/docs/build/llm-docs/payments/dashboard/account-settings/balances.md#add-funds-to-your-reserve-balance) to the account and then try doing the transfer.
