  ...
  try {
    ...
    if (role === "creator") {
      if (type === "basicInfo") {
        ...
        const { rows } = await query(
          "SELECT full_name,user_name, first_name, last_name, phone_number, email, social_platform_type, social_links, niche, about_me, created_at FROM creators WHERE user_id = $1",
          [user.user_id]
        );
    ...
    if (role === "freelancer") {
      ...
      if (type === "govtId") {
        ...
        const { rows } = await query(
          "SELECT gov_id_type, gov_id_front_image, gov_id_back_image, gov_id_number, verification_status, pan_card_number, pan_card_image_url FROM freelancer WHERE user_id = $1",
          [user.user_id]
        );

        if (!rows[0] || (!rows[0].gov_id_front_image && !rows[0].pan_card_image_url)) {
          logger.warn("Gov ID not uploaded");
          return next(new AppError("No govt ID found", 404));
        }

        const presignPath = (rawUrl) => {
          if (!rawUrl) return Promise.resolve(null);
          const parts = rawUrl.split("/");
          return createPresignedUrl(parts[0], parts.slice(1).join("/"), expirySeconds);
        };

        const [govFrontSignedUrl, govBackSignedUrl, panCardSignedUrl] = await Promise.all([
          presignPath(rows[0].gov_id_front_image),
          presignPath(rows[0].gov_id_back_image),
          presignPath(rows[0].pan_card_image_url),
        ]);
        ...
        return res.status(200).json({
          ...
          data: {
            userGovtIdFrontImage: govFrontSignedUrl,
            userGovtIdBackImage: govBackSignedUrl,
        ...
        try {
          const { rows } = await query(
            `UPDATE creators
             ...
             social_links=$5, niche=$6::text[] , email=$7, about_me=$8, updated_at=CURRENT_TIMESTAMP
             WHERE user_id=$9
      ...
      if (type === "govtId") {
        ...
        try {
          ...
          const { rows: freelancerGovInfo } = await query(
            `UPDATE freelancer
             SET gov_id_type=$1, gov_id_front_image=$2, gov_id_back_image=$3, gov_id_number=$4,
             ...
             WHERE user_id=$5
             RETURNING gov_id_type, gov_id_front_image, gov_id_back_image, gov_id_number`,
            [gov_id_type, gov_id_front_image, gov_id_back_image, gov_id_number, user.user_id]
          );
          ...
          return res.status(200).json({
            ...
            data: {
              ...freelancerGovInfo[0],
              gov_id_front_image: frontSignedUrl,
              gov_id_back_image: backSignedUrl,
            },
          });
        } catch (error) {
          await query("ROLLBACK");
...
const getAllFreelancers = async (req, res, next) => {
  ...
  try {
    ...
    if (minDays !== null && maxDays !== null) {
      deliverySubqueryFilter = `AND s2.min_delivery_days <= ${maxDays} AND s2.max_delivery_days >= ${minDays}`;
    } else if (minDays !== null) {
      deliverySubqueryFilter = `AND s2.max_delivery_days >= ${minDays}`;
    ...
    // include if service_start <= maxDays AND service_end >= minDays
    if (minDays !== null && maxDays !== null) {
      queryText += ` AND s.min_delivery_days <= $${paramCount} AND s.max_delivery_days >= $${paramCount + 1}`;
      queryParams.push(maxDays, minDays);
      paramCount += 2;
    } else if (minDays !== null) {
      queryText += ` AND s.max_delivery_days >= $${paramCount}`;
      queryParams.push(minDays);
      paramCount++;
    } else if (maxDays !== null) {
      queryText += ` AND s.min_delivery_days <= $${paramCount}`;
    ...
    if (minDays !== null && maxDays !== null) {
      countQuery += ` AND s.min_delivery_days <= $${countParamIndex} AND s.max_delivery_days >= $${countParamIndex + 1}`;
      countParams.push(maxDays, minDays);
      countParamIndex += 2;
    } else if (minDays !== null) {
      countQuery += ` AND s.max_delivery_days >= $${countParamIndex}`;
      countParams.push(minDays);
      countParamIndex++;
    } else if (maxDays !== null) {
      countQuery += ` AND s.min_delivery_days <= $${countParamIndex}`;
...
const getUserProfileProgress = async (req, res, next) => {
  ...
  try {
    ...
    const { rows: freelancerRows } = await query(
      "SELECT freelancer_thumbnail_image, date_of_birth, phone_number, profile_title, profile_image_url, gov_id_front_image, bank_account_no, bank_name, bank_ifsc_code, bank_branch_name, verification_status, reason_for_rejection FROM freelancer WHERE user_id=$1",
      [user.user_id]
    );
    ...
    if (freelancerRows.length > 0) {
      ...
      if (freelancer.gov_id_front_image) {
...
const getFreelancerForKYCApproval = async (req, res, next) => {
  try {
    ...
    // Query to get freelancers with Razorpay onboarding status
    const queryText = `
      SELECT
        f.freelancer_id,
        f.freelancer_full_name,
    ...
    const freelancersWithStatus = freelancers.map((freelancer) => {
      // Add computed field for next action
    ...
    return res.status(200).json({
      ...
      data: {
        freelancers: freelancersWithStatus,
        pagination: {
          currentPage: page,
          totalPages,
          totalCount,
...
const getFreeLancerByIdForAdmin = async (req, res, next) => {
  try {
    ...
    // Generate presigned URLs for government ID front and back images
    const presignGovImage = async (rawUrl, label) => {
      if (!rawUrl) return null;
      const idx = rawUrl.indexOf("/");
      if (idx === -1) { logger.warn(`Invalid govt ID ${label} URL format: ${rawUrl}`); return null; }
      try {
        return await createPresignedUrl(rawUrl.substring(0, idx), rawUrl.substring(idx + 1), expirySeconds);
      } catch (err) {
        logger.error(`Error generating signed URL for govt ID ${label}: ${err}`);
        return null;
      }
    };
    freelancer.gov_id_front_image = await presignGovImage(freelancer.gov_id_front_image, 'front');
    freelancer.gov_id_back_image = await presignGovImage(freelancer.gov_id_back_image, 'back');
    ...
    let masked_bank_account = null;
    if (freelancer.bank_account_no) {
      masked_bank_account = '****' + freelancer.bank_account_no.slice(-4);
    }
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        gov_id_front_image: freelancer.gov_id_front_image,
        gov_id_back_image: freelancer.gov_id_back_image,
        ...
        // Razorpay Routes onboarding status
        razorpay_onboarding: {
          status: freelancer.razorpay_account_status,
        ...
        // Address details
        address: freelancer.has_address ? {
          street: freelancer.street_address,
          city: freelancer.city,
          state: freelancer.state,
          postal_code: freelancer.postal_code
        } : null
      },
    });
  } catch (error) {
...
function buildFreelancerFilters(startIdx, { search, serviceTypes, minPrice, maxPrice, minDays, maxDays }) {
  const conditions = [];
  const params = [];
  let idx = startIdx;
  ...
  if (serviceTypes.length > 0) {
    conditions.push(`s.service_name = ANY($${idx}::text[])`);
  ...
  // delivery time filter (uses new integer columns)
  if (minDays != null && maxDays != null) {
    conditions.push(`s.min_delivery_days <= $${idx} AND s.max_delivery_days >= $${idx + 1}`);
    params.push(maxDays, minDays);
    idx += 2;
  } else if (minDays != null) {
    conditions.push(`s.max_delivery_days >= $${idx}`);
    params.push(minDays);
    idx++;
  } else if (maxDays != null) {
    conditions.push(`s.min_delivery_days <= $${idx}`);
    params.push(maxDays);
    idx++;
  }
...
const getAllfreelancersForcreator = async (req, res, next) => {
  ...
  try {
    ...
    const search = req.query.search || "";
    const minPrice = parseFloat(req.query.minPrice) || 0;
    const maxPrice = parseFloat(req.query.maxPrice) || 99999999.99;
    const serviceTypes = req.query.serviceType
      ? Array.isArray(req.query.serviceType)
        ? req.query.serviceType
        : [req.query.serviceType]
    ...
    // --- Lateral subquery: serviceType filter inside if needed ---
    const lateralServiceFilter = serviceTypes.length > 0
    ...
    const countFilters = buildFreelancerFilters(1, {
      search,
      serviceTypes,
      minPrice,
      maxPrice,
    ...
    const freelancers = await Promise.all(
      results.rows.map(async (f) => {
        ...
        if (f.min_delivery_days != null && f.max_delivery_days != null) {
          f.delivery_time = `${f.min_delivery_days}-${f.max_delivery_days} days`;
        } else {
          f.delivery_time = null;
        }
        // Clean up raw columns from response
        delete f.min_delivery_days;
        delete f.max_delivery_days;
