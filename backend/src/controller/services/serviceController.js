...
const addServicesByFreelancer = async (req, res, next) => {
  ...
  try {
    ...
    const { rows } = await query(
      `INSERT INTO services (freelancer_id, service_name, service_description, price, created_at, updated_at,delivery_duration)
...
const updateServiceByFreelancer = async (req, res, next) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      `UPDATE services
       SET service_name=$1, price=$2, service_description=$3, updated_at=$4, delivery_duration=$5
