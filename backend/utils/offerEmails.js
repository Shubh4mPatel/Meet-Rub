...
async function sendOfferReceivedEmail({ creatorEmail, creatorName, freelancerName, serviceTitle, amount, deliveryDays, freelancerUserId }) {
    ...
    const filled = fillTemplate(html, {
        ...
        amount: amount != null ? Number(amount).toFixed(2) : '—',
        delivery_days: deliveryDays || '—',
        offer_url: `${APP_URL}/chatbot?userId=${freelancerUserId}`,
...
async function sendHireRequestEmail({ creatorEmail, creatorName, freelancerName, serviceTitle, amount, deliveryDays, freelancerUserId }) {
    const html = fs.readFileSync(
...
async function sendHireAcceptedEmail({ creatorEmail, creatorName, freelancerName, serviceTitle, amount, deadline, freelancerUserId }) {
    ...
    const filled = fillTemplate(html, {
        ...
        payment_url: `${APP_URL}/chatbot?userId=${freelancerUserId}`,
        asset_base: ASSET_BASE,
        help_url: HELP_URL,
        privacy_url: PRIVACY_URL,
    });
...
async function sendHireDeclinedEmail({ creatorEmail, creatorName, freelancerName, freelancerUserId }) {
    ...
    const filled = fillTemplate(html, {
        creator_username: creatorName,
        freelancer_username: freelancerName,
        browse_url: `${APP_URL}/chatbot?userId=${freelancerUserId}`,
        asset_base: ASSET_BASE,
        help_url: HELP_URL,
        privacy_url: PRIVACY_URL,
    });
